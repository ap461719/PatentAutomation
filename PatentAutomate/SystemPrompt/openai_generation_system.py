...
def run_openai_generation(claims, method):
    """Run patent description generation using specified method with OpenAI API."""
    if method == 'full':
...
def main():
    ...
    try:
        print(f"Generating description using OpenAI API with {args.method} method...")
        ...
        if args.output:
            ...
            output_file = os.path.join(script_dir, f'{args.method}_description_openai.txt')
