...
# -----------------------------------------------------------------------------#
#  NEW: paragraph-level style-transfer helper                                   #
# -----------------------------------------------------------------------------#
STYLE_PROMPT = """
Polish the following patent paragraph so it reads like formal USPTO prose:
tighten legal language, vary sentence openings, remove repetition, and keep it 100 % factual.
Do NOT add new subject matter—rewrite only.

Paragraph:
"{para}"
"""

def stylize(para: str) -> str:
    """One additional Claude call that returns a polished version of `para`."""
    msgs = [
        {"role": "system", "content": "You are an expert patent editor."},
        {"role": "user",   "content": STYLE_PROMPT.format(para=para)},
    ]
    return chatgpt_chatbot(msgs).strip()

# -----------------------------------------------------------------------------#
#  Helper: TXT → DOCX (unchanged)                                               #
# -----------------------------------------------------------------------------#
...
# -----------------------------------------------------------------------------#
#  Full-shot generation (single polish pass)                                    #
# -----------------------------------------------------------------------------#
def generate_full_desc(claims: str) -> str:
    enhanced_prompt = """You are a specialized patent attorney assistant …
<original critical requirements text here>
